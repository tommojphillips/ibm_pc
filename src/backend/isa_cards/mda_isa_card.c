...
int isa_mda_write_io_byte(MDA* mda, uint16_t port, uint8_t value) {
	switch (port) {
...
int isa_mda_read_io_byte(MDA* mda, uint16_t port, uint8_t* value) {
	switch (port) {
		case MDA_BASE_ADDRESS + 0x0: // MDA Index
		case MDA_BASE_ADDRESS + 0x1: // MDA Data
		case MDA_BASE_ADDRESS + 0x2: // MDA Index
...
int isa_card_add_mda(ISA_BUS* bus, MDA* mda) {
	...
	int card = isa_bus_add_card(bus, "MDA Card");
